...
def cmd_update(args: argparse.Namespace) -> None:
    ...
    changed = False
    only_layer = getattr(args, "only", None)
    standalone = bool(getattr(args, "standalone", False))

    def branch_tip(ref: str) -> str:
        proc = run(["git", "rev-parse", "--verify", f"{ref}^{{commit}}"], cwd=root, check=False, capture=True)
        return (proc.stdout or "").strip() if proc.returncode == 0 else ""

    def is_ancestor(base: str, tip: str) -> bool:
        proc = run(["git", "merge-base", "--is-ancestor", base, tip], cwd=root, check=False, capture=True)
        return proc.returncode == 0

    def patch_ids_for_commits(commits: list[str]) -> list[str]:
        ids: list[str] = []
        for sha in commits:
            try:
                show = subprocess.run(
                ...
                pid_line = (pid.stdout or "").splitlines()[0] if (pid.stdout or "").strip() else ""
                ids.append(pid_line.split()[0] if pid_line else "")
            except Exception:
                ids.append("")
        return ids

    def branch_up_to_date_by_patch_id(*, base_ref: str, branch: str, desired_commits: list[str]) -> bool:
        if not branch_tip(branch):
            return False
        if not is_ancestor(base_ref, branch):
            return False
        actual_commits_raw = git(["rev-list", "--reverse", "--no-merges", f"{base_ref}..{branch}"], cwd=root)
        actual_commits = [c for c in actual_commits_raw.splitlines() if c]
        if len(actual_commits) != len(desired_commits):
            return False
        return patch_ids_for_commits(actual_commits) == patch_ids_for_commits(desired_commits)

    if filtered_mode(conf):
        entries = select_entries(conf, only_layer)
        # Source cut points live as local branches (so `rebase --update-refs` can keep them in sync).
        for entry in entries:
            target = resolve_commitish(root, entry.sha)
            cut_branch = cut_branch_for_entry(conf, entry)
            needs_move = branch_tip(cut_branch) != target
            if needs_move:
        ...
        if only_layer is not None and int(only_layer) > 1:
            prev_source = conf.entries[int(only_layer) - 2].sha
            prev_pr_base = conf.entries[int(only_layer) - 2].branch if not standalone else conf.base_remote_ref
        else:
        ...
        for entry in entries:
            ...
            base_for_branch = conf.base_remote_ref if standalone else prev_pr_base
            up_to_date = branch_up_to_date_by_patch_id(base_ref=base_for_branch, branch=pr_branch, desired_commits=commits)
            ...
            try:
                if current == pr_branch:
                    die(f"refusing to rewrite currently checked out branch: {current}")
                run(["git", "checkout", "-B", pr_branch, base_for_branch], cwd=root, capture=True)
                if commits:
                    run(["git", "cherry-pick", *commits], cwd=root, capture=True)
            except subprocess.CalledProcessError:
                run(["git", "cherry-pick", "--abort"], cwd=root, check=False, capture=True)
                run(["git", "checkout", orig], cwd=root, check=False, capture=True)
                die(
        ...
        if changed:
            run(["git", "checkout", orig], cwd=root, capture=True)
        return
    ...
    if standalone:
        ...
        for entry in entries:
            ...
            up_to_date = branch_up_to_date_by_patch_id(
                base_ref=conf.base_remote_ref,
                branch=pr_branch,
                desired_commits=commits,
            )
            if up_to_date:
                prev_source = entry.sha
                continue
            ...
            try:
                if current == pr_branch:
                    die(f"refusing to rewrite currently checked out branch: {current}")
                run(["git", "checkout", "-B", pr_branch, conf.base_remote_ref], cwd=root, capture=True)
                if commits:
                    run(["git", "cherry-pick", *commits], cwd=root, capture=True)
            except subprocess.CalledProcessError:
                run(["git", "cherry-pick", "--abort"], cwd=root, check=False, capture=True)
                run(["git", "checkout", orig], cwd=root, check=False, capture=True)
                die(f"failed to generate {pr_branch} (conflict or empty cherry-pick)")
            print(f"generated {pr_branch}")
            changed = True
            prev_source = entry.sha
    ...
    for entry in entries:
        target = resolve_commitish(root, entry.sha)
        needs_move = branch_tip(entry.branch) != target
        if needs_move:
...
def write_conf(root: Path, conf: StackConfig, entries: list[tuple[str, str, str, str]]) -> None:
    ...
    lines.append("#   body_check auto|off            Gate publication plans on PR-body lint (default: auto)")
    ...
    if conf.draft_mode != "stacked":
        ...
        lines.append("")
    if conf.body_check != "auto":
        lines.append(f"body_check {conf.body_check}")
        lines.append("")
