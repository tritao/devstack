...
def cmd_build(args: argparse.Namespace) -> None:
    ...
    if adapter is not None:
        ...
        # Generic fallback: support building any CMakePresets-based repo without an adapter.
        # (Adapter-only features like FreeCAD's --core presets are not available.)
        if not (root / "CMakePresets.json").is_file() and not (root / "CMakeUserPresets.json").is_file():
            note("build: no CMakePresets.json found; creating minimal CMakeUserPresets.json (debug/release)")
            ensure_basic_presets(root)

        if bool(args.core) or build_mode == "core":
            die("build: --core/--build-mode core requires an adapter (e.g. FreeCAD); pass --adapter or use --build-mode full")
        if build_mode != "full":
            die(f"invalid --build-mode: {build_mode} (expected: full)")

        if bool(args.clang_mold):
            toolchain = "clang-mold"

        if toolchain == "auto":
            toolchain = "clang-mold" if (have_cmd("clang") and have_cmd("clang++") and have_cmd("mold")) else "default"

        if toolchain not in ("default", "clang-mold"):
            die(f"invalid --toolchain: {toolchain} (expected: default|clang-mold|auto)")

        if toolchain == "clang-mold":
            # If the user already selected a clang-mold preset explicitly, leave it alone.
    ...
    if not presets and not adapter:
        # Some repos don't ship presets; try bootstrapping them (local-only) as a fallback.
        ensure_basic_presets(root)
        presets = load_cmake_presets(root)
    if not build_dir:
