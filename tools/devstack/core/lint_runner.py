...
def _resolve_tools_lint(root: Path, tools_from: str) -> ToolingPaths:
    ...
    if tools_from == "local":
        ...
        # A standalone devstack checkout does not necessarily carry the
        # target repository's lint scripts. Prefer the tool checkout, then
        # use the target repository when it owns tools/lint.
...
def run_lint(args: argparse.Namespace) -> None:
    ...
    adapter = try_load_adapter(root, os.environ.get("DEVSTACK_ADAPTER", "auto"))
    base_default = getattr(adapter, "lint_default_base_ref", lambda _root: "origin/main")(root) if adapter else "origin/main"
