...
def try_load_adapter(root: Path, name: str):
    """
    Like load_adapter(), but returns None when auto-detection fails.

    This is useful for commands that can fall back to generic behavior (e.g. base refs, build presets).
    """
    want = (name or "").strip() or os.environ.get("DEVSTACK_ADAPTER", "auto")
    want = want.strip() or "auto"
