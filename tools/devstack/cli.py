...
def build_parser() -> argparse.ArgumentParser:
    ...
    upd.add_argument("--only", type=int, help="Only update layer N (1-based).")
    upd.add_argument(
        "--standalone",
        action="store_true",
        help="Generate each PR branch onto the base branch using only that layer's commits (fork-friendly).",
    )
    ...
    ghs.add_argument(
        "--standalone",
        action="store_true",
        help="Use the stack base branch as the PR base for all layers (independent PRs).",
    )
    ghs.add_argument(
        "--draft",
        action="store_true",
    ...
    pl.add_argument(
        "--standalone",
        action="store_true",
        help="Generate the PR branch from base using only this layer's commits (does not depend on prior PR branches).",
    )
    ...
    ini = cmd(
        "init",
        "Bootstrap .devstack/ in this worktree (creates .devstack/stack.conf and ensures the configured PR bodies dir exists).",
        category="Worktrees",
    )
    ini.add_argument(
        "--copy",
        ...
        const="copy",
        default="copy",
        help="Deprecated (ignored; devstack runs from a single checkout).",
    )
    ini.add_argument(
        "--symlink",
        dest="mode",
    ...
    ini.add_argument("--force-script", action="store_true", help="Deprecated (ignored).")
    ...
    wti.add_argument("--symlink", action="store_true", help="Deprecated (ignored; no scripts are installed into the worktree).")
    ...
    wta.add_argument("--symlink", action="store_true", help="Deprecated (ignored; no scripts are installed into the worktree).")
    ...
    wts = cmd("wt-sync", "Sync stack config/PR bodies across existing worktrees.", category="Worktrees")
    ...
    wts.add_argument("--copy", dest="mode", action="store_const", const="copy", help="Deprecated (ignored).")
    wts.add_argument("--no-import", action="store_true")
    wts.add_argument("--import-from")
    wts.add_argument("--no-force-script", action="store_true", help="Deprecated (ignored).")
